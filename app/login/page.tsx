...
import { formatPhoneNumber, login, parsePhoneNumber, checkAdminAuth } from "@/lib/api/auth";
...
function AdminLoginInner() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!phone.trim()) { setError("Введите номер телефона"); setIsLoading(false); return; }
        if (!password.trim()) { setError("Введите пароль"); setIsLoading(false); return; }
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input id="phone" type="tel" value={phone}
                                   onChange={(e)=>{ setPhone(formatPhoneNumber(e.target.value)); if (error) setError(""); }}
                        ...
                        <div className="space-y-2">
                            ...
                            <Input id="password" type="password" value={password}
                                   ...
                                   placeholder="Введите пароль" required minLength={1} />
                        </div>
                        {error && (<Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>)}
                        <Button type="submit" className="w-full bg-[#aa0400] hover:bg-[#8a0300] text-white" disabled={isLoading}>
